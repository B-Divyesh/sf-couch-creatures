...
| 14 | Start a room, scan its QR code, then choose a lantern on the phone. | Pass |
...
| 7 | Move lanterns clear of three clay-storm strikes. | Pass |
...
| 9 | See how the fixed sample can finish or fail. | Pass |
...
| 18 | Enter the room code shown on the shared game, choose a lantern, then use the large direction buttons. | Pass |
| 4 | Privacy for shared play | Pass |
| 12 | Couch Creatures does not ask for names, accounts, photos, contacts, or location. | Pass |
| 9 | Run recovery and assist mode stay in this browser. | Pass |
| 16 | The relay stores a random room code, control presses, expiry times, and a one-way connection hash. | Pass |
| 7 | It deletes expired room and creation-limit records. | Pass |
| 12 | The demo uses separate browser storage and never contacts the phone-room relay. | Pass |
...
| 7 | Refreshing restores an active run as paused. | Pass |
...
| 13 | Start real play, then select **Start phone room** to show a QR code. | Pass |
...
| 15 | The demo has public replay buttons for a completed fixed rescue and a storm loss. | Pass |
...
| 9 | The exact public claim checks are listed in `.factory/claims.json`. | Pass |
...
| 6 | Assist mode remains enabled after reload. | Pass |
| 7 | Demo and real play use separate namespaces. | Pass |
| 6 | Shared-device play makes no cross-origin requests. | Pass |
| 15 | Phone rooms store random room codes, control presses, expiry times, and a one-way connection hash. | Pass |
| 8 | The relay removes expired room and creation-limit records. | Pass |
| 9 | Deploy `dist/` to static hosting with the included `staticwebapp.config.json`. | Pass |
...
| 9 | Visual direction and original-image provenance are recorded in `.factory/design.md`. | Pass |
| 6 | The demo details are in `.factory/demo.md`. | Pass |
